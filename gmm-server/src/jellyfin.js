...
class GestorJellyfin {
  ...
  async actualizar() {
    if (!this.usuarioId) {
      const respuestaUsuarios = await this.pedir("/Users");
      const usuarios = await respuestaUsuarios.json();
      if (!usuarios || !usuarios.length || !usuarios[0].Id) {
        throw new Error("Jellyfin no devolvió ningún usuario para consultar la biblioteca.");
      }
      this.usuarioId = usuarios[0].Id;
    }
    const campos = "ProviderIds,MediaSources,ProductionYear,DateCreated";
    /* El endpoint global /Items también devuelve películas huérfanas de rutas
       antiguas. Al indicar el usuario, Jellyfin limita el resultado a la
       biblioteca visible y evita duplicados después de cambiar una unidad. */
    const respuesta = await this.pedir("/Items?UserId=" + encodeURIComponent(this.usuarioId) +
      "&Recursive=true&IncludeItemTypes=Movie&Fields=" +
